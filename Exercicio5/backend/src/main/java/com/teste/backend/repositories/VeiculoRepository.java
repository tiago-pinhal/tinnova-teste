...
public interface VeiculoRepository extends PagingAndSortingRepository<Veiculo, String> {
    ...
    List<Veiculo> findAllByOrderByCreatedDesc();
    
    @Query("select count(*) from Veiculo v where v.vendido = false")
    long qtdVeiculosNaoVendidos();

    @Query(value = "SELECT distinct CONVERT(CONCAT(LEFT(v.ano,3), '0'),integer) as decada, count(v.ano) as qtd FROM VEICULO v WHERE v.ano BETWEEN CONVERT(CONCAT(LEFT(v.ano,3), '0'),integer) AND CONVERT(CONCAT(LEFT(v.ano,3), '0'),integer) + 10 group by v.ano", nativeQuery = true)
